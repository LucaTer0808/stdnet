...
class Eval_thread():
    def __init__(self, loader, save_dir, dat, cuda):
        self.loader = loader
        self.cuda = cuda
        self.dat = dat
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        self.logfile = os.path.join(save_dir, 'result_{}.txt'.format(dat))
    ...
    def run(self):
        ...
        return mae, s, max_f, Fm

    def Eval_mae(self):
        print('eval[MAE]:{} dataset.'.format(self.dat))
    ...
    def Eval_fmeasure(self):
        print('eval[FMeasure]:{} dataset.'.format(self.dat))
        beta2 = 0.3
        ...
        with torch.no_grad():
            ...
            for pred, gt in self.loader:
                i += 1
                if i % 100 == 0:
                    print('processing {}/{} image'.format(i, len(self.loader)))
                if self.cuda:
                    pred = trans(pred).cuda()
                    gt = trans(gt).cuda()
    ...
    def Eval_auc(self):
        print('eval[AUC]:{} dataset.'.format(self.dat))
    ...
    def Eval_Smeasure(self):
        print('eval[Smeasure]:{} dataset.'.format(self.dat))
    ...
    def Eval_AP(self, prec, recall):
        ...
        print('eval[AP]:{} dataset.'.format(self.dat))
