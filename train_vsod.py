"""
Training script for VSOD models
...
from dataloader_vsod import prepare_train_data
...
parser = argparse.ArgumentParser(description='Train STDNet')
...
parser.add_argument('--train-lst', type=str, default='dataset/train_video_lst.txt', 
        help='name of training dataset')
parser.add_argument('--val-lst', type=str, default='dataset/val_video_lst.txt', 
...
parser.add_argument('--nstdc', type=int, default=1, 
        help='repeat times in stdc module')
parser.add_argument('--tid', nargs='+', default=['cv', 'cd', 'ad'],
        help='temporal operations, in [cv, cd, ad]')
parser.add_argument('--scales', nargs='+', default=[0, 1, 1],
...
parser.add_argument('--beta', default=None, nargs="+", 
...
parser.add_argument('--lr-reduce', type=float, default=0.05, 
        help='reduction factor for learning rate of pretrained weights')
parser.add_argument('--lr-type', type=str, default='multistep', 
        help='learning rate strategy [cosine, multistep]')
parser.add_argument('--lr-steps', type=str, default='30-50', 
...
def main(running_file):
    ...
    if args.checkinfo:
        #count_paramsM, count_flopsM = measure_model(model, 224, 224)
        #print('Model size: %f MB\nFlops: %f M' % (count_paramsM, count_flopsM))
    ...
    ### Define optimizer
    ## if there the block is pretrained
    if args.preload is not None:
        groupname = ['conv weigths', 'bn weights', 'relu weights', 'block weights']
        weights = model.get_stage2_weights()
        args.lr_factors = [1.0, 1.0, 1.0, args.lr_reduce]
        wds = [args.wd, 0.1 * args.wd, 0., 0.5 * args.wd]
    else:
        groupname = ['conv weigths', 'bn weights', 'relu weights']
        weights = model.get_normal_weights()
        args.lr_factors = [1.0, 1.0, 1.0]
        wds = [args.wd, 0.1 * args.wd, 0.]
    param_groups = [{
        'params': weights[i],
        'lr': args.lr, # the lr will be adjusted in adjust_learning_rate_stage2 function
        'weight_decay': wds[i]
    ...
    ### Load Data
    args.val_datasets = ['DAVSOD', 'VOS', 'DAVIS']
    args.val_cuts = [args.val_cut, 1.1, 1.1]
    train_loader, _val_loader = prepare_train_data(args)

    best_mae = {}
    for val_name in args.val_datasets:
        best_mae[val_name] = 10000

    ### Create log file
    log_file = os.path.join(args.savedir, '%s_log.txt' % args.model)
    with open(log_file, 'a') as f:
        f.write('{}\n'.format(args.val_datasets))
    ...
    if args.resume or (args.resume_from is not None):
        ...
        if checkpoint is not None:
            ...
            cp_mae = checkpoint['best_mae']
            if isinstance(cp_mae, dict):
                best_mae = cp_mae
    ...
    ### Train
    saveID = None
    print('current best: {}'.format(best_mae))

    lossFunc = bce_ssim_loss
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        val_mae_info = []
        best_mae_info = []
        for val_name, val_loader in zip(args.val_datasets, _val_loader):
            mae = validate(val_loader, val_name, model, epoch, running_file, args)
            is_best[val_name] = mae <= best_mae[val_name]
            best_mae[val_name] = min(mae, best_mae[val_name])
            val_mae_info.append('{:.4f}'.format(mae))
            best_mae_info.append('{:.4f}'.format(best_mae[val_name]))
        val_mae_info = ' | '.join(val_mae_info)
        best_mae_info = ' | '.join(best_mae_info)

        log = ("Epoch %02d/%02d: mae %s || best MAE %s" + \
              " | train loss %.4f=(%.4f+%.4f+%.4f) | lr %s | Time %s\n") \
              % (epoch, args.epochs, val_mae_info, best_mae_info, \
...
def train(train_loader, model, lossFunc, optimizer, epoch, running_file, running_lr, args, scale):
    ...
    for i, data in enumerate(train_loader):
        ...
        if args.use_cuda:
            ...
            input, target = data[:2]
            mask = data[2] if args.beta is not None else None
        ...
        loss, bce, ssim, iou = lossFunc(output, target, mask)
        ...
        for loss_id, loss_value in enumerate([loss.item(), bce, ssim, iou]):
            losses[loss_id].update(loss_value)
        ...
        if i % args.print_freq == 1:
            runinfo = str(('Epoch: [{0:0%dd}/{1:0%dd}][{2:0%dd}/{3:0%dd}]\t' \
                      ...
                      'Loss {loss.val:.4f}(={loss1.val:.4f}+{loss2.val:.4f}+{loss3.val:.4f})' + \
                      '(avg:{loss.avg:.4f})\t' + \
                      'lr {lr}\t').format(
                          epoch, args.epochs, i, 
                          len(train_loader), scale,
                          batch_time=batch_time, data_time=data_time, 
                          loss=losses[0], loss1=losses[1], loss2=losses[2], loss3=losses[3], 
                          lr=running_lr))
            print(runinfo)
...
def validate(val_loader, val_name, model, epoch, running_file, args):
    batch_time = AverageMeter()
    mae = AverageMeter()
    ...
    print(' * {}: MAE {mae.avg:.3f}'.format(val_name, mae=mae))
